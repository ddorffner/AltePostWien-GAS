...
powershell -Command "docker rm -f altepost-persistent-gen 2>$null"
...
powershell -Command "docker run -d --name altepost-persistent-gen --user root -p 6379:6379 -v C:\redis-data:/data --restart unless-stopped redis:latest redis-server /data/redis.conf"

echo Redis container started. Checking logs...
timeout /t 2 > nul
powershell -Command "docker logs altepost-persistent-gen"
echo.
echo To verify data loading, run: docker exec -it altepost-persistent-gen redis-cli
pause