...
{
	#region PluginInfo
	[PluginInfo(Name = "ShellExecuteAdv", Category = "Windows", Version = "Advanced", AutoEvaluate = true, Help = "Executes a program (w/o parameters) and returns the output, errors and status", Tags = "process, command line, cmd", Author="zeos", Credits = "alg - addonpack integration")]
	#endregion PluginInfo
	public class ShellExecuteAdv : IPluginEvaluate, IDisposable
	{
		...
		ISpread<string> FInputIn;
		
		[Input("SendInput", IsBang=true, IsSingle = true)]
		IDiffSpread<bool> FSendInput;
		...
		public void Evaluate(int SpreadMax)
		{
			if((FExecuteIn.IsChanged && FExecuteIn[0]) && (!string.IsNullOrEmpty(FFileIn[0])) && !FRunning)
			{
				...
				FProcess.StartInfo.WindowStyle = ProcessWindowStyle.Normal;
			...
			if(FRunning) {
				...
				FRunningOut[0] = FProcess.Responding;
				
				if(FSendInput.IsChanged && FSendInput[0]) 
				{
					for(int i = 0; i< FInputIn.SliceCount; i++)
					    if (sw.BaseStream.CanWrite)
					    {
					          sw.WriteLine(FInputIn[i]);
